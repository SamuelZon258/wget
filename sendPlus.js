...
let PUSH_TITLE_ARR = [];
if (process.env.PUSH_TITLE_ARR) {
    PUSH_TITLE_ARR = process.env.PUSH_TITLE_ARR.split('&');
}
...
if (UidFileexists) {
    ...
    console.log(TempCKUid)
}

function getPtPin(text, desp) {
    for (let i = 0; i < PUSH_TITLE_ARR.length; i++) {
        if (text.indexOf(PUSH_TITLE_ARR[i]) > -1) {
            if (send && TempCKUid.length > 0) {
                for (let i = 0; i < TempCKUid.length; i++) {
                    let zh = `账号${i + 1}`
                    if (desp.indexOf(zh) > -1 && TempCKUid[i] != null && TempCKUid[i].pt_pin.Uid != "") {
                        return TempCKUid[i].pt_pin
                    }
                }
            }
            return null
