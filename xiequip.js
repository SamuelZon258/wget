...
function getXQIP(timeout = 0) {
    return new Promise((resolve) => {
        $.get({
            url: `http://op.xiequ.cn/IpWhiteList.aspx?uid=${xquid}&ukey=${xqukey}&act=get`
        }, async(err, resp, data) => {
...
function UpdateIP(ip, timeout = 0) {
    return new Promise((resolve) => {
        $.get({
            url: `http://op.xiequ.cn/IpWhiteList.aspx?uid=${xquid}&ukey=${xqukey}&act=del&ip=all`
        }, async(err, resp, data) => {
            $.get({
                url: `http://op.xiequ.cn/IpWhiteList.aspx?uid=${xquid}&ukey=${xqukey}&act=add&ip=${ip}`
            }, async(err, resp, data) => {
